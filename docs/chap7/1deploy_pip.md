...
作业分为Build、test、codeanalysis、artifactory、deploy部分

`jobs/build.yml` 包含两个作业模板，分别是普通的构建模板和docker 镜像构建模板。
...
`jobs/codeanalysis.yml `包含两个作业模板，分别为扫描作业和获取扫描结果。
...
**`jobs/artifactory.yml` 包含两个作业，制品上传与下载。**
...
**`jobs/deploy.yml` 发布基于k8s的发布和回滚配置。**
